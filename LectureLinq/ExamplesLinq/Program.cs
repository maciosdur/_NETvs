...

{
    ...
    public class XYZ
    {
        ...
        // metoda z parametrami — będziemy ją wywoływać przez reflection
        public string Describe(string prefix, int repeat)
        {
            return string.Join(" ", Enumerable.Repeat($"{prefix} -> {Name}#{Value}", repeat));
        }

        // dodatkowa metoda zwracająca int (do przykładu Invoke zwracającego typ prosty)
    ...
    class Program
    {
        ...
        static void Main()
        {
            ...
            if (xyzType != null)
            {
                ...
                object obj2 = Activator.CreateInstance(xyzType, new object[] { "ReflectionInstance", 99 });
                ...
                if (methodInfo != null)
                {
                    ...
                    object[] parameters = new object[] { "TEST", 2 };
